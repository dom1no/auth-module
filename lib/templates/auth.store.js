...
export default {
  ...
  mutations: {
    ...
    AUTH_LOGIN_ERROR (state, errors) {
      state.isLoading = false
      state.formErrors = errors.response.data
    }
  },
  ...
  actions: {
    ...
    // Login
    async login ({ commit, dispatch }, { fields, endpoint = '<%= options.login.endpoint %>' } = {}) {
      // Send credentials to API
      commit('AUTH_LOGIN_LOAD')
      try {
